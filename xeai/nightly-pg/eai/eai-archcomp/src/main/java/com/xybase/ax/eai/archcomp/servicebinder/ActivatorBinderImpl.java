 ...
 * **************************************************************************************
 * Oct 27, 2015	9:12:59 AM			Jovi Rengga Salira		Initial Creation
 * Oct 27, 2015	9:12:59 AM			Jovi Rengga Salira		Dynamics Concept Implementation
 * 	- Group by Integration Type[Synchronize, Asynchronous]
 *  - Synchronize Type
 * **************************************************************************************
 */
...
public class ActivatorBinderImpl implements Activator<Message>, ServiceBinder,
		DisposableBean {

	private final static Logger log = LogManager
			.getLogger(ActivatorBinderImpl.class);
	...
	private Map<String, Executor> bindedService;

	private String binderKey;
	private String defaultBinder;

	public ActivatorBinderImpl() {
		// TODO Auto-generated constructor stub
		bindedService = new HashMap<String, Executor>();
	}

	@Override
	public Message activate(Message message) {
		// TODO Auto-generated method stub
		log.info(this.binderKey + ":"
				+ message.getHeaders().get(this.binderKey));
		if (!message.getHeaders().containsKey(binderKey)
				&& !bindedService.containsKey(message.getHeaders().get(
						binderKey)))
			return bindedService.get(defaultBinder).execute(message);
		else
			return bindedService.get(
	...
	public void setBaseCtxResources(Larik baseContextResource) {
		...
		log.info("baseContext is Alive! of {"
				+ this.baseCtxResources.toString() + "}");
	}
	...
	public String add(String binderValue, String contextFile) {
		...
		if (bindedService.containsKey(binderValue))
			...
			this.bindedService.put(binderValue, new ExecutorImpl(baseContext,
	...
	public String stop() {
		...
		baseContext.destroy();
		baseContext = null;
		for (Entry<String, Executor> entry : this.bindedService.entrySet()) {
	...
	public String info() {
		...
		for (int i = 0; i < this.baseCtxResources.size(); i++) {
			infos.add(BusRspUtil.asPair("BASE", this.baseCtxResources.get(i)));
		}
