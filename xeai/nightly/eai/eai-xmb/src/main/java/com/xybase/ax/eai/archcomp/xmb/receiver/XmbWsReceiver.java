...
public class XmbWsReceiver implements Receiver<String> {
	...
	public String invoke(String soapRequest) {
		...
		GenericMessage<XMBTextMessage> messageOut = new GenericMessage<XMBTextMessage>(
				xmbMessage, header);
		extractors.setContext(messageOut);
		AuditLog log = new AuditLog(messageOut);
		log.setPayload(soapRequest);
		log.setEndpoint("3.2.2.XEAI.2|" + beanName);
		log.setCorrelationId(StringUtil.cast(extractors.extract(auditLogConfig
				.getCorrelation())));
		log.setAuditParam(StringUtil.cast(extractors.extract(auditLogConfig
				.getParameterized())));
		...
		try {
			...
			soapResponse.replaceAll("<refId/>", "<refId>"
					+ xmbMessage.getRefId() + "</refId>");
		} catch (Exception e) {
			xmbMessage.setRefId(StringUtil.emptyChar);
			log.setSeverity(e.getCause().toString());
			auditLogDao.audit(log);
			soapResponse.replaceAll("<message><refId/></message>", "<message>"
					+ e.getCause().toString() + "</message>");
		}
		return soapResponse;
	}
}
